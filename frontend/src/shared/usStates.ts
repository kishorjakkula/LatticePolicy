...
export function regionsForCountry(country?: string | null): RegionOption[] {
  return normalizeCountryCode(country) === 'CA' ? CANADA_PROVINCES : US_STATES
}

export function defaultRegionForCountry(country?: string | null): string {
  return normalizeCountryCode(country) === 'CA' ? 'ON' : 'NY'
}

export function isRegionInCountry(region: string | undefined | null, country?: string | null): boolean {
  const normalizedRegion = normalizeRegionCode(region)
  if (!normalizedRegion) return false
  return regionsForCountry(country).some((entry) => entry.code === normalizedRegion)
}
